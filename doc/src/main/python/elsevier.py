...
def createDoiTable(conn):
	...
	cur.execute("create table elsevier_records ( path_pii varchar(150), doii varchar(150), pii varchar(150), path varchar(150), title varchar(250), authors varchar(250), dtd_version varchar(20), dtd_file varchar(20), abstract_only boolean, has_abstract boolean, primary key(path_pii) );")
	conn.commit()
	cur.close()
...
def insertElsevierArticle(attrDict, conn):
	...
	try:
		...
		print('XXX Exception'  + str(e) + "|" + str(attrDict)   )
	conn.commit()
